        ...
        <section class="pt-3 pb-4" id="count-stats">
            <div class="container">
                ...
                <div class="d-flex gap-2 mb-4 align-items-center">
                    <a class="btn btn-primary mb-0" href="{{ route('replacement_list_member', ['Id_Report_Replacement' => $listReport->Id_Report_Replacement]) }}">
                        <i class="material-symbols-rounded text-sm">arrow_back</i> Back
                    </a>

                    <div class="d-flex align-items-center gap-2 ms-auto">
                        @if($prevReportId)
                            <a href="{{ route('replacement_list_member.detail', ['Id_List_Report_Replacement' => $prevReportId]) }}" class="btn btn-outline-primary mb-0" title="Previous Report">
                        ...
                        @if($nextReportId)
                            <a href="{{ route('replacement_list_member.detail', ['Id_List_Report_Replacement' => $nextReportId]) }}" class="btn btn-outline-primary mb-0" title="Next Report">
                ...
                <div class="d-flex justify-content-between align-items-center mt-3 mb-3">
                    @if($prevReportId)
                        <a href="{{ route('replacement_list_member.detail', ['Id_List_Report_Replacement' => $prevReportId]) }}" class="btn btn-outline-primary">
                    ...
                    @if($nextReportId)
                        <a href="{{ route('replacement_list_member.detail', ['Id_List_Report_Replacement' => $nextReportId]) }}" class="btn btn-outline-primary">
    ...
    <script>
        ...
        const CONFIG = {
            ...
            colors: {
                ...
                comment: { text: 'white', bg: '#800080', border: 'transparent' }
            }
        };
        ...
        async function downloadPdf() {
            ...
            link.download = '{{ $member->Name_Member }}-{{ $listReport->display_name }}.pdf';
            link.click();
        }
        ...
        async function submitReport() {
            ...
            fetch(`{{ route('replacement_list_member.submit', ['Id_List_Report_Replacement' => $listReport->Id_List_Report_Replacement]) }}`, {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                body: formData
            }).then(res => {
                if (res.ok) {
                    alert('Prosedur pengganti berhasil disubmit!');
                    location.reload();
                } else {
                    alert('Gagal menyubmit prosedur pengganti');
                }
            });
        }
        ...
        function renderCommentToPDF(page, div, x, y, font) {
            ...
            page.drawRectangle({
                ...
                width: boxWidth,
                height: boxHeight,
                color: PDFLib.rgb(0.502, 0, 0.502),
                opacity: 1
            });
